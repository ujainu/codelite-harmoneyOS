#!/usr/bin/env python3
"""F-UI-3.1d: register-safe wxString cleanup slice in libcodelite_app CreateGUIControls.

Window 0x5da484–0x5da4a0 only. Rules:
  - never adjust SP (wxString temp at [sp,#216])
  - never touch x19/x20 (this + nearby frame)
  - save x0-x3 + x8 to callee-saved x23-x27 around OH_LOG_Print
  - cave trampolines: bl stub; stub runs orig insn; b resume

Uses appended RX cave @ 0x772930 (same as F-UI-3.1c).
"""
...
MARKER = b"FUI31DWS\x00"

TRACES: list[tuple[int, int, str, int]] = [
    (0x5DA484, 0xF9406FE8, "[FUI_FRAME_FLOW] after LoadMenuBar cleanup begin", 0),
    (0x5DA488, 0xF9044260, "[FUI_FRAME_FLOW] after m_mainMenuBar store", 0),
    (0x5DA490, 0x940613C0, "[FUI_FRAME_FLOW] before wxString dtor", 0),
    (0x5DA4A0, 0x940613C0, "[FUI_FRAME_FLOW] cleanup finished", 0),
]
...
class A64:
    ...
    def save_scratch(self) -> None:
        # x23-x27 are callee-saved; OH_LOG must preserve them across the call.
        # x23 holds x8 (wxString temp handle for mov x0,x8 @ 0x5da48c).
        self.emit(0xAA0003F8)  # mov x24, x0
        self.emit(0xAA0103F9)  # mov x25, x1
        self.emit(0xAA0203FA)  # mov x26, x2
        self.emit(0xAA0303FB)  # mov x27, x3
        self.emit(0xAA0803F7)  # mov x23, x8

    def restore_scratch(self) -> None:
        self.emit(0xAA1803E0)  # mov x0, x24
        self.emit(0xAA1903E1)  # mov x1, x25
        self.emit(0xAA1A03E2)  # mov x2, x26
        self.emit(0xAA1B03E3)  # mov x3, x27
        self.emit(0xAA1703E8)  # mov x8, x23
...
def main() -> None:
    ...
    if MARKER in data:
        print(f"[f-ui-3.1d] already patched {CL}")
        return
    if b"FUI31CFLOW" in data:
        raise SystemExit("[f-ui-3.1d] libcodelite has F-UI-3.1c patch — restore golden first")
    extend_rx_cave(data)
    stub_va = CAVE
    for site_va, orig, msg, continue_va in TRACES:
        ...
        if is_trace_stub_bl(cur, site_va):
            print(f"[f-ui-3.1d] skip {msg} @ {site_va:#x}")
            continue
        if cur != orig:
            raise SystemExit(f"[f-ui-3.1d] unexpected @ {site_va:#x}: {cur:#x} expected {orig:#x}")
        ...
        print(f"[f-ui-3.1d] {msg} @ {site_va:#x} -> {stub_va:#x}")
        stub_va += (len(stub) + 15) & ~15
    marker_off = va_to_offset(data, CAVE + CAVE_FILE_BYTES - 32)
    data[marker_off : marker_off + len(MARKER)] = MARKER
    CL.write_bytes(data)
    print(f"[f-ui-3.1d] patched {CL} ({len(TRACES)} slice probes)")
