#!/usr/bin/env bash
# F-UI-3.3b-2: DetachMenuBar enter probe after F-UI-3.3c clMainFrame fix.
set -euo pipefail
...
ensure_device() {
  ...
  echo "[f-ui-3.3b-2] ABORT: no hdc device (start MateBook Pro emulator first)" >&2
  exit 1
}
...
echo "[f-ui-3.3b-2] Step 1 — golden + F-UI-3.3c fix + DetachMenuBar enter probe"
...
echo "[f-ui-3.3b-2] Step 2 — cold install"
ensure_device
...
echo "[f-ui-3.3b-2] Step 3 — poll DetachMenuBar enter (${F_UI33B2_ATTEMPTS:-3} attempts)"
LOG=""
for attempt in $(seq 1 ${F_UI33B2_ATTEMPTS:-3}); do
  LOG="$ROOT/docs/logs/f-ui-3-3b2-verify-$(date +%Y%m%d-%H%M%S)-a${attempt}.txt"
  ...
  echo "[f-ui-3.3b-2] attempt $attempt: DetachMenuBar enter not seen yet"
done

grep -E 'FUI_FRAMEBASE|FUI_XRC.*LoadMenuBar|MenuBar handler|THREAD_BLOCK|SIGSEGV' "$LOG" | tail -40 || true
echo "[f-ui-3.3b-2] log saved: $LOG"
echo "[f-ui-3.3b-2] gates:"
set +e
bash "$ROOT/host/fw2-hap/scripts/check-f-ui-3-3b2-gates.sh" "$LOG"
exit $?
