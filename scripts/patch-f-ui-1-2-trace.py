#!/usr/bin/env python3
"""F-UI-1.2: [FUI_TRACE] probes at CreateGUIControls call sites (trace only).

Uses an appended RX cave (NOT 0x4FE65C — that is live CompilerMainPageBase code).
Stubs save x0/x1 via callee-saved regs; do not adjust sp (CreateGUIControls may run
with a tight stack during OnSurfaceCreated).
"""
...
VA_OH_LOG = 0x762900
CAVE = 0x772930  # appended RX tail (VA hole before RW @ 0x773930)
CAVE_FILE_BYTES = 0x1000
NOP = b"\x1f\x20\x03\xd5"
TRACES: list[tuple[int, int, str, int]] = [
    (0x5DA2B4, 0x94003143, "[FUI_TRACE] GetBestXButtonSize enter", 0x5DA2B8),
    (0x5DA2B8, 0x11001015, "[FUI_TRACE] GetBestXButtonSize return", 0),
    (0x5DA424, 0x940030E7, "[FUI_TRACE] GetBestXButtonSize enter", 0x5DA428),
    (0x5DA428, 0xF94002A8, "[FUI_TRACE] GetBestXButtonSize return", 0),
    (0x5DA480, 0x94064710, "[FUI_TRACE] LoadMenuBar enter", 0),
    (0x5DA51C, 0x940624F5, "[FUI_TRACE] AUI Update enter", 0),
    (0x5DABE8, 0x94064552, "[FUI_TRACE] MainBook enter", 0),
]
...
def extend_rx_cave(data: bytearray) -> None:
    ...
    print(f"[f-ui-1.2] extended RX cave +0x{CAVE_FILE_BYTES:x} @ VA 0x{CAVE:x}")
...
class A64:
    ...
    def save_x0_x1(self) -> None:
        self.emit(0xA9BF07E0)  # stp x0, x1, [sp, #-16]!

    def restore_x0_x1(self) -> None:
        self.emit(0xA8C107E0)  # ldp x0, x1, [sp], #16
    ...
    def hilog(self, msg_va: int, fmt_va: int, tag_va: int) -> None:
        ...
        self.emit(0x72A01E02)  # domain 0xA0000
        self.adrp(3, tag_va)
...
def main() -> None:
    ...
    for site_va, orig, msg, continue_va in TRACES:
        ...
        if is_trace_stub_bl(cur, site_va):
            print(f"[f-ui-1.2] skip {msg} @ {site_va:#x} (already patched)")
            continue
        if cur != orig:
            raise SystemExit(f"[f-ui-1.2] unexpected insn @ {site_va:#x}: {cur:#x} expected {orig:#x}")
        ...
        print(f"[f-ui-1.2] {msg} @ {site_va:#x} -> stub {stub_va:#x} resume {resume:#x}")
        stub_va += (len(stub) + 15) & ~15

    CL.write_bytes(data)
    print(f"[f-ui-1.2] patched {CL} ({len(TRACES)} probes, cave={CAVE:#x})")
