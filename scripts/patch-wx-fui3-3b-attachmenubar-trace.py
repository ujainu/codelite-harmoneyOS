#!/usr/bin/env python3
"""F-UI-3.3b: AttachMenuBar / wxMenuBar::Attach slice (post F-UI-3.3c fix).

Apply patch-wx-fui3-3c-getmenubar-vtable-offset-fix.py first. Do NOT combine with
F-UI-3.3a probes (0x43FE98) or FUI33FB framebase patch — separate cave/marker.

Probes: DetachMenuBar pre-ret, AttachMenuBar enter/attach/return, wxMenuBar::Attach enter.
Never patch inside wxFrameBase::SetMenuBar (conflicts with F-UI-3.3c).
"""
...
MARKER = b"FUI33BB\x00"

# Probes only inside DetachMenuBar / AttachMenuBar / wxMenuBar::Attach — never SetMenuBar body.
TRACES: list[tuple[int, int, str, bool]] = [
    (0x43FE30, 0xF9400BF3, "[FUI_FRAMEBASE] post DetachMenuBar", False),
    (0x43FE40, 0xA9BE7BFD, "[FUI_FRAMEBASE] AttachMenuBar enter", True),
    (0x43FE68, 0xF9015693, "[FUI_FRAMEBASE] AttachMenuBar Attach blr", False),
    (0x43FE70, 0xA8C27BFD, "[FUI_FRAMEBASE] AttachMenuBar return", False),
    (0x659414, 0xD10203FF, "[FUI_MENUBAR] Attach enter", False),
]
...
class A64:
    ...
    def spill_regs(self) -> None:
        self.emit(0xD10103FF)
        self.emit(0xA90107E0)
        self.emit(0xA9020FE2)
        self.emit(0xF90023E8)

    def unspill_regs(self) -> None:
        self.emit(0xA94107E0)
        self.emit(0xA9420FE2)
        self.emit(0xF94023E8)
        self.emit(0x910103FF)
...
def patch_core(path: Path) -> None:
    ...
    marker_off = va_to_offset(data, CAVE + 0x580)
    if MARKER in data:
        print(f"[f-ui-3.3b] already patched attach slice in {path}")
        return
    ...
    for i, (site_va, orig, msg, use_stack) in enumerate(TRACES):
        ...
        if is_bl_to_cave(cur, site_va):
            print(f"[f-ui-3.3b] skip {msg} @ {site_va:#x}")
            stub_va += 0x80
            continue
        if cur != orig:
            raise SystemExit(
                f"[f-ui-3.3b] unexpected @ {site_va:#x}: {cur:#x} expected {orig:#x} ({msg})"
            )
        resume = site_va + 4
        stub = build_log_stub(
        ...
        print(f"[f-ui-3.3b] {msg} @ {site_va:#x} -> {stub_va:#x}")
        stub_va += 0x80

    data[marker_off : marker_off + len(MARKER)] = MARKER
    path.write_bytes(data)
    print(f"[f-ui-3.3b] patched {path} ({len(TRACES)} attach probes)")
