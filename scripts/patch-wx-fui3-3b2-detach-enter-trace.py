#!/usr/bin/env python3
"""F-UI-3.3b-2: single DetachMenuBar enter probe (post F-UI-3.3c fix).

Apply patch-wx-fui3-3c-getmenubar-vtable-offset-fix.py first.
One probe at wxFrameBase::DetachMenuBar enter (0x43FE08) to distinguish:
  Case A — hang in SetMenuBar before DetachMenuBar is entered
  Case B — hang inside DetachMenuBar (enter seen, post-Detach not reached)

Never patch inside wxFrameBase::SetMenuBar. Marker FUI33B2.
"""
...
def patch_core(path: Path) -> None:
    ...
    marker_off = va_to_offset(data, CAVE + 0x500)
    if MARKER in data:
        print(f"[f-ui-3.3b-2] already patched Detach enter in {path}")
        return
    ...
    for i, (site_va, orig, msg, use_stack) in enumerate(TRACES):
        ...
        if is_bl_to_cave(cur, site_va):
            print(f"[f-ui-3.3b-2] skip {msg} @ {site_va:#x}")
            stub_va += 0x80
            continue
        if cur != orig:
            raise SystemExit(
                f"[f-ui-3.3b-2] unexpected @ {site_va:#x}: {cur:#x} expected {orig:#x} ({msg})"
            )
        resume = site_va + 4
        stub = build_log_stub(
        ...
        print(f"[f-ui-3.3b-2] {msg} @ {site_va:#x} -> {stub_va:#x}")
        stub_va += 0x80

    data[marker_off : marker_off + len(MARKER)] = MARKER
    path.write_bytes(data)
    print(f"[f-ui-3.3b-2] patched {path} ({len(TRACES)} detach-enter probe)")
