#!/usr/bin/env bash
# F-UI-3.3a-2: GetMenuBar virtual target + core/app thunk probes.
set -euo pipefail
...
if [[ "${F_UI33A2_SKIP_GOLDEN_GATE:-0}" != "1" ]]; then
  echo "[f-ui-3.3a-2] Step 0 — F-UI-2 golden gate"
  step0_ok=0
  for attempt in 1 2 3; do
    ...
    hdc -t "$TARGET" shell aa start -a EntryAbility -b "$BUNDLE"
    prelog="$ROOT/docs/logs/f-ui-2-golden-precheck-$(date +%Y%m%d-%H%M%S)-33a2-a${attempt}.txt"
  ...
  [[ "$step0_ok" -eq 1 ]] || { echo "[f-ui-3.3a-2] ABORT F-UI-2 gate" >&2; exit 1; }
fi

echo "[f-ui-3.3a-2] Step 1 — golden + vtable + GetMenuBar blr target (BLR_ONLY=${F_UI33A2_BLR_ONLY:-0})"
bash "$ROOT/host/fw2-hap/scripts/restore-f-ui-2-golden-snapshot.sh"
python3 "$ROOT/host/fw2-hap/scripts/patch-f-ui-3-2b-vtable-dispatch-trace.py"
F_UI33A2_BLR_ONLY="${F_UI33A2_BLR_ONLY:-0}" python3 "$ROOT/host/fw2-hap/scripts/patch-wx-fui3-3a2-getmenubar-trace.py"
# Optional: app weak GetMenuBar thunk (fires on any GetMenuBar — off by default)
if [[ "${F_UI33A2_PATCH_APP_GETMB:-0}" == "1" ]]; then
  python3 "$ROOT/host/fw2-hap/scripts/patch-cl-fui3-3a2-getmenubar-trace.py"
fi
...
echo "[f-ui-3.3a-2] Step 2 — cold install"
pack_hap
hdc -t "$TARGET" uninstall "$BUNDLE" 2>/dev/null || true
hdc -t "$TARGET" install "$HAP"
hdc -t "$TARGET" shell hilog -r
...
echo "[f-ui-3.3a-2] Step 3 — poll GetMenuBar slice (${F_UI33A2_SLICE_ATTEMPTS:-5} attempts)"
LOG=""
for slice_attempt in $(seq 1 ${F_UI33A2_SLICE_ATTEMPTS:-5}); do
  LOG="$ROOT/docs/logs/f-ui-3-3a2-verify-$(date +%Y%m%d-%H%M%S)-a${slice_attempt}.txt"
  if [[ "$slice_attempt" -gt 1 ]]; then
    hdc -t "$TARGET" shell hilog -r
    if [[ "${F_UI33A2_RETRY_REINSTALL:-1}" == "1" ]]; then
      pack_hap
      hdc -t "$TARGET" uninstall "$BUNDLE" 2>/dev/null || true
      hdc -t "$TARGET" install "$HAP"
    fi
  ...
  : > "$LOG"
  deadline=$(($(date +%s) + ${F_UI33A2_POLL_TIMEOUT:-90}))
  while [[ $(date +%s) -lt "$deadline" ]]; do
    hdc -t "$TARGET" shell hilog -x 2>/dev/null >> "$LOG" || true
    if grep -qF '[FUI_GETMB] post GetMenuBar cmp' "$LOG"; then
      break
    fi
    if grep -qF '[FUI_GETMB] target=' "$LOG"; then
      break
    fi
    if grep -qF '[FUI_VTABLE] slot=' "$LOG" && ! grep -qE 'SIGSEGV|cppcrash-com\.codelite' "$LOG"; then
      break
    fi
    if grep -qE 'SIGSEGV|cppcrash-com\.codelite' "$LOG"; then
      ...
      echo "[f-ui-3.3a-2] attempt $slice_attempt: crash before GetMenuBar target — retry"
      break
    fi
    sleep 2
  done
  hdc -t "$TARGET" shell hilog -x 2>/dev/null >> "$LOG" || true
  if grep -qE 'FUI_GETMB|FUI_VTABLE' "$LOG" && ! grep -qE 'SIGSEGV|cppcrash-com\.codelite' "$LOG"; then
    break
  fi
  if grep -qF '[FUI_GETMB] target=' "$LOG"; then
    break
  ...
  if grep -qE 'SIGSEGV|cppcrash-com\.codelite' "$LOG"; then
    echo "[f-ui-3.3a-2] attempt $slice_attempt: crash — retry"
  else
    echo "[f-ui-3.3a-2] attempt $slice_attempt: no GetMenuBar slice yet"
  fi
done

grep -E 'FUI_GETMB|FUI_VTABLE|FUI_XRC.*LoadMenuBar return|THREAD_BLOCK|SIGSEGV' "$LOG" | tail -45 || true

echo "[f-ui-3.3a-2] log saved: $LOG"
echo "[f-ui-3.3a-2] GetMenuBar gates:"
set +e
bash "$ROOT/host/fw2-hap/scripts/check-f-ui-3-3a2-gates.sh" "$LOG"
exit $?
