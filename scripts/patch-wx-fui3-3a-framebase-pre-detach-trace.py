#!/usr/bin/env python3
"""F-UI-3.3a: wxFrameBase::SetMenuBar pre-Detach slice (post-return probes).

Avoid patching virtual blr sites (caused SIGSEGV on device); use post-call sites:
  0x43FE98 — post GetMenuBar() cmp x0, x19
  0x43FEB0 — post DetachMenuBar() ldr x8, [x20]

Patches libwx_ohosu_core @ cave 0x6F7E00; marker FUI33FA (not FUI33FB).
Apply with patch-f-ui-3-2b-vtable-dispatch-trace.py only — never combine with FUI33FB.
"""
...
MARKER = b"FUI33FA\x00"

# (site_va, orig_insn, message) — all mid-frame, stack spill only
TRACES: list[tuple[int, int, str]] = [
    (0x43FE98, 0xEB13001F, "[FUI_FRAMEBASE] post GetMenuBar cmp"),
    (0x43FEB0, 0xF9400288, "[FUI_FRAMEBASE] post DetachMenuBar"),
]
...
def patch_core(path: Path) -> None:
    ...
    if b"FUI33FB" in path.read_bytes():
        raise SystemExit("[f-ui-3.3a] FUI33FB marker present — restore golden first")
    ohlog = nm_ohlog_plt(path)
    data = bytearray(path.read_bytes())
    marker_off = va_to_offset(data, CAVE + 0x600)
    if MARKER in data:
        print(f"[f-ui-3.3a] already patched pre-Detach slice in {path}")
        return

    msgs = [m for _, _, m in TRACES]
    ...
    for i, (site_va, orig, msg) in enumerate(TRACES):
        ...
        if is_bl_to_cave(cur, site_va):
            print(f"[f-ui-3.3a] skip {msg} @ {site_va:#x}")
            stub_va += 0x80
            continue
        if cur != orig:
            raise SystemExit(f"[f-ui-3.3a] unexpected @ {site_va:#x}: {cur:#x} expected {orig:#x}")
        resume = site_va + 4
        stub = build_log_stub(stub_va, ohlog, orig, resume, msg_vas[i], fmt_va, tag_va)
        ...
        print(f"[f-ui-3.3a] {msg} @ {site_va:#x} -> {stub_va:#x}")
        stub_va += 0x80

    data[marker_off : marker_off + len(MARKER)] = MARKER
    path.write_bytes(data)
    print(f"[f-ui-3.3a] patched {path} ({len(TRACES)} pre-Detach probes)")
