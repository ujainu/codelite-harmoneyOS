#!/usr/bin/env python3
"""F-UI-3.3d: DetachMenuBar / AttachMenuBar / wxMenuBar::Attach slice only.

Apply patch-wx-fui3-3c-getmenubar-vtable-offset-fix.py first.
Never patch wxFrameBase::SetMenuBar body (SIGSEGV with 3.3c).
Marker FUI33D.
"""
...
TRACES: list[tuple[int, int, str, bool]] = [
    (0x43FE08, 0xA9BE7BFD, "[FUI_FRAMEBASE] DetachMenuBar enter", True),
    (0x43FE20, 0xF9400008, "[FUI_FRAMEBASE] DetachMenuBar pre virtual blr", False),
...
def patch_core(path: Path) -> None:
    ...
    if MARKER in data:
        print(f"[f-ui-3.3d] already patched detach/attach slice in {path}")
        return
    ...
    for i, (site_va, orig, msg, use_stack) in enumerate(TRACES):
        ...
        if is_bl_to_cave(cur, site_va):
            print(f"[f-ui-3.3d] skip {msg} @ {site_va:#x}")
            stub_va += 0x80
            continue
        if cur != orig:
            raise SystemExit(
                f"[f-ui-3.3d] unexpected @ {site_va:#x}: {cur:#x} expected {orig:#x} ({msg})"
            )
        resume = site_va + 4
        stub = build_log_stub(
        ...
        print(f"[f-ui-3.3d] {msg} @ {site_va:#x} -> {stub_va:#x}")
        stub_va += 0x80

    data[marker_off : marker_off + len(MARKER)] = MARKER
    path.write_bytes(data)
    print(f"[f-ui-3.3d] patched {path} ({len(TRACES)} detach/attach probes)")
